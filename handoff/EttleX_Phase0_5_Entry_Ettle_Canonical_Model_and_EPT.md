...
**Product:** EttleX  
**Purpose:** Implement the canonical in-memory Ettle/EP model including full CRUD semantics, deterministic RT/EPT traversal, invariant enforcement, and bootstrap inspection/export to enable dogfooding and subsequent Phase 1 capabilities.  
...
- Bootstrap inspection/export tools:
  - `inspect show <ettle_id>` prints WHY/WHAT/HOW (including HOW Gherkin)
  - `inspect export <leaf_id> [--leaf-ep <ep_id>]` writes a realisation bundle Markdown file derived from EPT
...
Phase 0.5 MUST include a minimal bootstrap tool (binary) exposing:

##### 7.1 `inspect show <ettle_id>`
...
Output MUST be stable-order and suitable for human review.

##### 7.2 `inspect export <leaf_id> [--leaf-ep <ep_id>]`

Writes a realisation bundle Markdown derived from EPT.
...
Export MUST fail explicitly on traversal/validation errors and MUST be deterministic (byte-identical for same input).
...
6) Provide a minimal bootstrap executable (`ettlex_inspect`) with commands:
   - `show <ettle_id>`
   - `show-ept <leaf_ettle_id> [--leaf-ep <ep_id>]`
   - `export <leaf_ettle_id> [--leaf-ep <ep_id>]`
...
Scenario: Exporting the same leaf twice yields identical bytes  
  ...
  When I export X twice  
  Then the two exported outputs are identical  
